...
bool oneDistance(string first, string second){
...
int bfs(const string start, const string end, map<string,vector<string> >&neighbors){
    queue<string>words;
    int level=2;
    int cnt=neighbors[start].size();
    for(int i=0;i<cnt;i++){
        words.push(neighbors[start][i]);
    }
    set<string>endStates(neighbors[end].begin(),neighbors[end].end());
    set<string>visted(neighbors[start].begin(),neighbors[start].end());
    while(!words.empty()){
        string word=words.front();
        if(endStates.count(word))return level+1;
        for(string::size_type i=0;i<neighbors[word].size();i++){
            string t=neighbors[word][i];
            if(!visted.count(t)) words.push(t);
        } 
        words.pop();
        if(--cnt==0){
            level++;
...
int ladderLength(string start, string end, unordered_set<string> &dict){
    ...
    for(int i=0;i<words.size();i++){
        ...
        for(int j=0;j<words.size();j++){
            if(oneDistance(words[i],words[j]))neighbors[words[i]].push_back(words[j]);
        }
    }
    if(neighbors[start].empty()||neighbors[end].empty())return 0;
    int len=bfs(start,end,neighbors);
